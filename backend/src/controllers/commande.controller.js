...
exports.createCommande = async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      const unitPrice = produit.onSale && produit.promoPrice
        ? produit.promoPrice
        : produit.price;
...
exports.getShopCommandes = async (req, res) => {
  try {
    ...
    const filtered = commandes.map(cmd => {
      const shopItems = cmd.items.filter(item =>
...
exports.getCommandeById = async (req, res) => {
  try {
    const commande = await Commande.findById(req.params.id)
      .populate('buyerId', 'nom prenom email phone');
...
exports.getStats = async (req, res) => {
  try {
    ...
    const byStatus = await Commande.aggregate([
